...
`encode-lint-cli` 是[前端编码规范工程化](https://github.com/zjydipingxian/zhongjiayao-fe-spec)的配套 Lint 工具，可以为项目一键接入
规范、一键扫描和修复规范问题，保障项目的编码规范和代码质量。
...
在项目根目录执行 `encode-lint-cli init`，即可一键接入规范，为项目安装规范 `Lint` 所需的依赖和配置。
...
如果你的 TS 项目 commit 卡口和 `encode-lint-cli scan` 运行时间很长，可以通过如下在 `.eslintrc.js` 中增加以下配置提升性能：
