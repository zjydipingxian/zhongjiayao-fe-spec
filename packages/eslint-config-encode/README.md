...
级别，仅将必要问题报告为 `error`，引用方式为在相应配置的 `null-eslint-config-encode` 后面加上 `/essential`，如对 `JS React` 项目为
`null-eslint-config-encode/essential/react`、对 `TS Vue` 项目为 `null-eslint-config-encode/essential/typescript/vue`
